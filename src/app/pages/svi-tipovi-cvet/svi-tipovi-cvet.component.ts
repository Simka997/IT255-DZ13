...
export class SviTipoviCvetComponent implements OnInit {
  ...
  private _getCvetTipovi() {
    ...
    this._http.get('http://localhost:8080/IT255-DZ13/getTipoveCveca.php', {headers: headers}).subscribe((data): any => {
  ...
  public removeCvetTip(event: Event, item: Number) { 
    ...
    headers.append('Content-Type', 'application/x-www-form-urlencoded');
    headers.append('token', localStorage.getItem('token'));
     this._http.get('http://localhost:8080/IT255-DZ13/deleteCvetTip.php?id='+item, {headers:headers})  .subscribe( data => {
